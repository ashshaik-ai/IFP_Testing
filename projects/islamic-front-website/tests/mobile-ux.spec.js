...
test.describe('Homepage — hub-tabs navigation', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('./');
    await waitForPageReady(page, '.hub-tab');
  });
...
test.describe('Homepage — mobile carousels (@mobile)', () => {
  ...
  test.beforeEach(async ({ page }) => {
    await page.goto('./');
    await waitForPageReady(page, '.achievements-grid');
  });
...
test.describe('Student Guidance — mobile stream collapse (@mobile)', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.locator('#if-app-tabs [data-screen="explore"]').click();
    await page.waitForTimeout(200);
  });

  test('stream prompt #sgStreamPrompt is visible after opening Explore', async ({ page }) => {
    const prompt = page.locator('#sgStreamPrompt');
    await expect(prompt).toBeAttached();
    await expect(prompt).toBeVisible();
  });

  test('all 4 stream sections hidden on Explore load (activeStream=none)', async ({ page }) => {
  ...
  test('hash deep-link #mpc auto-selects MPC and shows section', async ({ page }) => {
    ...
    await waitForPageReady(page, '#sgSearch');
    await page.waitForTimeout(400);
...
test.describe('Regression — no JS console errors', () => {
  ...
  for (const path of ['./', './student-guidance.html']) {
    test(`no JS errors on ${path}`, async ({ page }) => {
      ...
      await waitForPageReady(page, path.includes('student-guidance') ? '#sgSearch' : '.hub-tab');
